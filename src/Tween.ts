			...
			if (this._repeat > 0) {
				...
				for (property in this._valuesStartRepeat) {
					...
					if (this._yoyo && this._delayTime === 0) {
						...
						const currentValue = this._object[property]
				...
				const nextStartTime = this._startTime + durationAndDelay * completeCount
				...
				if (this._yoyo && this._delayTime === 0) {
					...
					const newPortion = Math.min(newElapsedTime / this._duration, 1)
					const newValue = this._easingFunction(newPortion)
